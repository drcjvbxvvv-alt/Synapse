...
type NetworkNode struct {
	...
	Hosts        []string          `json:"hosts,omitempty"`        // Phase C: Ingress hostnames
	HasHPA       bool              `json:"hasHPA,omitempty"`       // §7.3: HPA attached
	HPAMin       int32             `json:"hpaMin,omitempty"`       // §7.3: minReplicas
	HPAMax       int32             `json:"hpaMax,omitempty"`       // §7.3: maxReplicas
}

type NetworkEdge struct {
	Source       string  `json:"source"`
	Target       string  `json:"target"`
	Kind         string  `json:"kind,omitempty"`   // "" (static) | "ingress" | "istio-flow"
	...
	PolicyStatus string  `json:"policyStatus,omitempty"` // "policy-allow"|"policy-deny"|"policy-restricted"
	PolicyName   string  `json:"policyName,omitempty"`   // NetworkPolicy name that triggered this status
	// Phase F: Cilium Hubble real flow data
	HubbleFlowRate  float64 `json:"hubbleFlowRate,omitempty"`  // forwarded flows/s
	HubbleDropRate  float64 `json:"hubbleDropRate,omitempty"`  // 0.0-1.0
	HubbleDropReason string `json:"hubbleDropReason,omitempty"` // e.g. "POLICY_DENIED"
}
...
func GetClusterNetworkTopology(
	...
	// 6.5 Enrich workload nodes with HPA info
	type hpaKey struct{ ns, kind, name string }
	hpaMap := map[hpaKey]struct{ min, max int32 }{}
	for _, ns := range namespaces {
		...
		for _, hpa := range hpaList.Items {
			...
			hpaMap[hpaKey{ns, ref.Kind, ref.Name}] = struct{ min, max int32 }{min, hpa.Spec.MaxReplicas}
		}
	}
	for i := range nodes {
		if nodes[i].Kind != "Workload" {
			continue
		}
		key := hpaKey{nodes[i].Namespace, nodes[i].WorkloadKind, nodes[i].Name}
		if info, ok := hpaMap[key]; ok {
			nodes[i].HasHPA = true
			nodes[i].HPAMin = info.min
			nodes[i].HPAMax = info.max
		}
	}
