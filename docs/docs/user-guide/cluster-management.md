...
Synapse 支持同时管理多个 Kubernetes 集群。本文档介绍如何添加、管理和监控集群。
...
kubectl create serviceaccount synapse -n kube-system

# 创建 ClusterRoleBinding
kubectl create clusterrolebinding synapse-admin \
  --clusterrole=cluster-admin \
  --serviceaccount=kube-system:synapse

# 获取 Token (Kubernetes 1.24+)
kubectl create token synapse -n kube-system --duration=8760h
```
...
:::warning 注意
删除集群只会从 Synapse 移除管理记录，不会影响实际的 Kubernetes 集群。
:::
...
为 Synapse 创建专用的 ServiceAccount，按需授予权限：

```yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRole
metadata:
  name: synapse-readonly
rules:
- apiGroups: [""]
...
1. **检查网络**: 确保 Synapse 能访问 API Server
