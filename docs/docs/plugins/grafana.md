...
Synapse 深度集成 Grafana，支持在统一界面中嵌入监控面板、自动同步 Dashboard 和数据源。

## 两种部署模式

Synapse 支持两种 Grafana 部署方式：

| 模式 | 适用场景 | 说明 |
|------|---------|------|
| **内置 Grafana** | Docker Compose 快速部署 | 随 Synapse 一起启动，自动初始化 Service Account 和 API Token |
| **外置 Grafana** | 已有 Grafana 实例、Kubernetes 部署 | 连接已有的独立 Grafana 服务，手动配置连接信息 |

:::tip
无论哪种模式，Grafana 连接配置（URL 和 API Key）都在 Synapse **系统设置** 页面管理，存储在数据库中。
:::
...
部署完成后，在 Synapse **系统设置** → **Grafana 配置** 中填写：
...
如果你已有独立部署的 Grafana 实例（如通过 Helm 安装在 Kubernetes 集群中，或独立的虚拟机部署），可以将其与 Synapse 对接。

### 前置要求

- Grafana 9.0+ 已部署并可访问
- Synapse 所在网络能够访问 Grafana 服务地址
- 拥有 Grafana 管理员账号（用于创建 Service Account）

### 步骤一：配置 Grafana 允许嵌入

Synapse 通过 iframe 嵌入 Grafana 面板，需要在 Grafana 侧开启以下配置。
...
Synapse 通过 Grafana API 自动同步数据源和 Dashboard，需要一个具有 **Admin** 角色的 Service Account Token。
...
7. 填写 Token 名称（如 `synapse-token`），过期时间可设为 **No expiration**
8. 点击 **Generate token**
...
SA_RESULT=$(curl -s -X POST \
  ...
  -d '{"name":"synapse","role":"Admin","isDisabled":false}' \
...
TOKEN_RESULT=$(curl -s -X POST \
  -u "${GRAFANA_USER}:${GRAFANA_PASSWORD}" \
  -H "Content-Type: application/json" \
  -d '{"name":"synapse-token","secondsToLive":0}' \
...
1. 登录 Synapse，进入 **系统设置** → **Grafana 配置**
2. 填写连接信息：
...
注意：填写的地址需要是 **Synapse 后端**能访问到的地址，而非浏览器端地址。
:::
...
Synapse 内置了预配置的 Dashboard JSON（通过 Go embed 嵌入二进制）：
...
点击 **同步 Dashboard** 按钮，Synapse 会自动：
1. 在 Grafana 中创建 `Synapse` 文件夹
2. 将所有内置 Dashboard 导入到该文件夹（幂等操作，可重复执行）

#### 同步数据源

Synapse 会根据已配置 Prometheus 监控的集群，自动在 Grafana 中创建对应的 Prometheus 数据源：
...
```
浏览器 → Synapse(:8080) → Grafana(http://grafana:3000)
                              → Prometheus(http://prometheus:9090)
```

### 场景二：Synapse 在 Docker，Grafana 在 Kubernetes

```
浏览器 → Synapse(:8080) → Grafana(http://<NodeIP>:30300)
                              → Prometheus(http://<NodeIP>:30900)
```

需要确保 Synapse 容器网络能访问到 Kubernetes 集群的 NodePort 或 Ingress。

### 场景三：全部在 Kubernetes 中

```
浏览器 → Ingress → Synapse → Grafana(http://grafana.monitoring.svc:3000)
...
2. 检查 Synapse 到 Grafana 的网络连通性
3. 检查 API Key 是否有效（未过期、角色为 Admin）
4. 查看 Synapse 后端日志中的错误信息

```bash
# Docker Compose 查看日志
docker compose logs synapse | grep -i grafana
```
