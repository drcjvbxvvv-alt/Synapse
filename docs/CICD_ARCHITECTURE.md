...
| **可插拔** | 進階場景支援接入外部 CI 引擎（GitLab CI / Jenkins / Tekton / Argo Workflows / GitHub Actions）透過 `CIEngineAdapter` 介面。詳見 [ADR-015](./adr/ADR-015-CI-Engine-Adapter-Pattern.md)（M18a ✅完成） |
...
| 概念 | 說明 |
|------|------|
| `Pipeline` | Pipeline 定義，包含 Steps DAG、觸發條件、環境變數引用。**頂層全域實體**，不綁定特定叢集或命名空間 |
...
CREATE TABLE pipelines (
    ...
    notify_on_success  TEXT,        -- JSON 陣列 notify_channel_ids
    ...
    updated_at  DATETIME,
    deleted_at  DATETIME,
    UNIQUE KEY uq_pipeline_name (name, deleted_at)  -- 全域唯一，無叢集前綴
);
...
CREATE TABLE pipeline_secrets (
    id          BIGINT PRIMARY KEY AUTO_INCREMENT,
    scope       VARCHAR(20) NOT NULL,       -- global / environment / pipeline
    scope_ref   BIGINT,                     -- environment_id 或 pipeline_id
...
- `environment` scope：該 Environment 所屬 Pipeline 的 Writer
...
pipeline_runs ─── cluster_id ──► clusters（直接綁定，無 Environment 中介）
...
| 表 | 變更類型 | 說明 |
|---|---------|------|
| `pipelines` | **已修改** | 頂層實體，移除 cluster_id/namespace，全域唯一 name |
...
| `environments` | **已移除** | Environment 抽象層已移除，PipelineRun 直接帶 cluster_id + namespace |
| `promotion_history` | **已移除** | 同上 |
| `image_scan_results` | 修改 | 新增 `scan_source`、`pipeline_run_id`、`step_run_id` |
| `approval_requests` | 修改 | 擴充 `ResourceKind` 值域 + `pipeline_run_id` |
...
| 需求 | 選擇 | 理由 |
|------|------|------|
| CI 執行引擎 | K8s Job（原生，Native Adapter）+ 可插拔外部引擎（M18a ✅）| 零額外元件為預設；複雜場景透過 `CIEngineAdapter` 介面接入 GitLab CI / Jenkins / Tekton / Argo / GitHub。詳見 [ADR-015](./adr/ADR-015-CI-Engine-Adapter-Pattern.md) 與 [CI_ENGINE_ADAPTERS.md](./CI_ENGINE_ADAPTERS.md) |
...
- [x] Step Image 白名單（PlatformAdmin API）✅ GET/PUT /system/pipeline/allowed-images — PipelineService.GetAllowedImages/UpdateAllowedImages + system_settings 儲存
...
**Week 7：前端**
- [x] `PipelineList.tsx` / `PipelineEditor.tsx`（卡片 + YAML 雙模式）✅ 頂層 Pipeline 列表 + YAML 編輯器
- [x] `PipelineRunDetail.tsx`（DAG 進度圖）✅ ReactFlow DAG + StepRun 狀態 + SSE 日誌
- [x] `StepLogViewer.tsx`（SSE + 歷史）✅ SSE 串流 + 歷史日誌 Drawer
- [x] `PipelineEnvironments.tsx` ⚠️ 已移除（v3.0 決策 ADR-012，改以 cluster_id 直接隔離）
- [x] `PipelineSecretManager.tsx` ✅ Secret CRUD Drawer + scope(pipeline/global) + 名稱正則驗證 + 值永不顯示
- [x] `PipelineAllowedImages.tsx` ✅ Drawer + glob pattern 列表 + GET/PUT /system/pipeline/allowed-images
...
- [x] Git Provider 連線設定 UI ✅ GitProviderSettings.tsx — CRUD Modal + Webhook URL 顯示 + Token 重新生成，位於 SystemSettings 頁 git-providers tab

### M14.1 — Project 層（v3.1，未來里程碑）

> **背景：** M14 完成後，Webhook 可觸發 Pipeline，但 `git_providers` 直接關聯 `pipelines`，無法區分同一 Provider 下的不同 repo（例如 gitlab.com/org/service-a 與 gitlab.com/org/service-b 觸發不同 Pipeline）。

- ✅ `projects` 資料表（`git_provider_id`、`repo_url`、`default_branch`、`description`）+ migration 012
...
- [x] Repository / Tag 瀏覽 UI ✅ RegistryBrowser.tsx — 雙欄 Drawer（Repository 列表 + Tag 列表），FolderOpenOutlined 按鈕嵌入 RegistrySettings
...
- [x] 前端整合：GitOps 應用管理 UI ✅ GitOpsApps.tsx — 合併列表(native+ArgoCD) + Create/Edit/Delete/Sync/Diff + 路由 /clusters/:id/gitops/apps + Sidebar 已更新
- [x] ArgoCD 代理保留並明確邊界 — source 互斥規則 + deploy step exclusion

### M17 — 環境流水線（5 週）

> **⚠️ 已取消（v3.0 決策）：** Environment 抽象層過度設計。Pipeline Run 直接帶 `cluster_id` + `namespace`，叢集命名慣例（如 `cluster_dev`）取代環境晉升流程。詳見 ADR-012。
...
- [x] 前端：RolloutList / RolloutDetail / RolloutStatusWidget ✅ RolloutList.tsx (479L) + RolloutDetail.tsx (549L) + RolloutStatusWidget.tsx (155L)，路由 /clusters/:id/rollouts 已完整配置
...
- **Consequences：**
  ...
  - 🔓 **Escape hatch：** §6 明列「若需複雜 DAG 或既有 Tekton 資產，Synapse 支援以 Tekton 外掛方式執行」——保留未來整合空間。**此 escape hatch 已由 [ADR-015](./adr/ADR-015-CI-Engine-Adapter-Pattern.md)（CI Engine Adapter Pattern）具體落實，M18a ✅ 完成框架實作。**
- **追蹤：** `internal/services/pipeline/executor.go`、M13a 里程碑；外部引擎整合見 [ADR-015](./adr/ADR-015-CI-Engine-Adapter-Pattern.md) 與 [CI_ENGINE_ADAPTERS.md](./CI_ENGINE_ADAPTERS.md)。
...
### ADR-012 — 移除 Environment 抽象層

**日期：** 2026-04-15
**狀態：** 已採用

**決策：** 移除 `environments` 表與晉升流水線（M17），`PipelineRun` 直接帶 `cluster_id` + `namespace`。

**理由：**
- 大多數團隊直接用叢集命名（`cluster_dev`, `cluster_prod`）做隔離，不需要額外的 Environment 抽象層
- Environment 表造成觸發 Run 必須先建立 Environment 才能執行，增加設定步驟
- auto_promote / approval_required 的功能可以用 Pipeline YAML 的 `approval` step type 替代
- 簡化資料模型：`Pipeline → PipelineRun(cluster_id, namespace) → StepRun`

**影響：**
- 刪除：`internal/models/environment.go`、`internal/handlers/environment_handler.go`、`internal/services/environment_service.go`
- 刪除：migration `009` 中的 `environments` + `promotion_history` 表（migration `011` 清除）
- 更新：`TriggerRunRequest` 直接帶 `cluster_id` + `namespace`
- 更新：`TriggerRule`（webhook 觸發）帶 `cluster_id` + `namespace`
- 前端：移除 `PipelineEnvironments.tsx`，新增 `TriggerRunModal.tsx`（觸發時選叢集）
...
**決策：** 將 CI 執行引擎抽象為 `CIEngineAdapter` 介面，Synapse 同時支援 Native（內建 K8s Job，預設）+ 外部引擎（GitLab CI / Jenkins / Tekton / Argo Workflows / GitHub Actions）；每條 Pipeline 獨立選擇引擎。

**理由：**
- 客戶既有 Jenkins / GitLab / Tekton 投資龐大，強制遷移不切實際
- 遵循 `CLAUDE.md §8` Observer Pattern，與 Istio / KEDA / Cilium 等可選元件處理方式一致
- 保留 Native Engine 作為零依賴 fast path，同時解鎖「Synapse 當統一戰情室」的商業模式

**影響：**
- 新增 `internal/services/pipeline/engine/`（Adapter 介面、Factory、Native 實作）
- 新增 `models.CIEngineConfig`（加密連線設定）
- `Pipeline` 擴充 `EngineType` / `EngineConfigID`（向後相容，預設 `native`）
- 新增 HTTP API `/api/v1/ci-engines`（CRUD + 跨引擎探測）

**完整內容：** [ADR-015](./adr/ADR-015-CI-Engine-Adapter-Pattern.md)
**使用指南：** [CI_ENGINE_ADAPTERS.md](./CI_ENGINE_ADAPTERS.md)
**實作紀錄：** [M18a_IMPLEMENTATION_RECORD.md](./M18a_IMPLEMENTATION_RECORD.md)
