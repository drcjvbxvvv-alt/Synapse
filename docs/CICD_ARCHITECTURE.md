...
> 版本：v2.3 | 日期：2026-04-09 | 狀態：設計中（重構 + 補強 + 模型分工 + 熱力圖）
...
25. [Claude Code 模型分工建議](#25-claude-code-模型分工建議)
...
| 任務類型 | 建議模型 | 說明 |
|---------|---------|------|
| **ADR 撰寫（§21 ADR-001~009）** | **Opus** | 架構決策留痕 |
...
§21 列出 9 條 ADR（ADR-001~009），全部建議使用 **Opus** 撰寫。
特別是 ADR-003（Pipeline Secret 加密）、ADR-005（Webhook 安全模型）、ADR-007（GitOps 與 ArgoCD 邊界），這三條錯一條都會導致架構返工。
...
| 🟠 P1 | §21 所有 ADR（9 條）| 6 個月後「為什麼這樣設計」無人能答 |
...
- 任何 DB migration SQL
- §21 的 9 條 ADR 與 §23 的 STRIDE 威脅建模
