...
> 版本：v2.4 | 日期：2026-04-14 | 狀態：設計中（重構 + 補強 + 模型分工 + 熱力圖 + 生產對齊）
...
## v2.4 變更記錄（v2.3 → v2.4 補強）

v2.3 完成模型分工後，v2.4 基於生產集群方案文檔 v1.1 進行架構對齊：

| 新增/修改章節 | 解決的問題 |
|--------------|------------|
| §3 新增 Argo Rollouts 行 | 生產方案採用 Rollouts v1.8.4，CICD 架構未整合 |
| §4 新增灰度發布差距行 | 補齊灰度發布、Gateway API 路由、Cilium 網路策略差距 |
| §7.4.1 vSphere CSI 拓撲約束 | PVC WaitForFirstConsumer 對多 Step 共享的影響 |
| §7.7.1 Istio Ambient 環境注意事項 | Pipeline Pod 在 Ambient mesh 下的網路行為規範 |
| §7.7.2 Cilium NetworkPolicy 自動偵測 | 優先使用 CiliumNetworkPolicy 獲得 L7 策略能力 |
| §9.1 新增 Rollouts 相關 Step 類型 | deploy-rollout / rollout-promote / rollout-abort / rollout-status |
| §12.1 擴展邊界表 | ArgoCD / 原生 GitOps / Argo Rollouts 三方共存規則 |
| §17.1 新增 Rollout 失敗模式 | Rollout 分析失敗、灰度卡住的處理 |
| §19.1 版本兼容矩陣 | 各組件最低/推薦版本對齊生產方案 |
| §20 路線圖新增 Rollouts 整合 | M13c（Rollouts 整合，2 週） |
| ADR-010 | 灰度發布採 Argo Rollouts 代理模式 |
| §25.2 新增 Rollouts 模型指派 | M13c 逐任務模型分工 |

---

## v2.3 變更記錄（v2.2 → v2.3 補強）

v2.2 完成 Milestone 逐任務模型指派後，v2.3 補上跨 Milestone 整合視圖：

| 新增章節 | 解決的問題 |
|----------|------------|
| §25.8 Milestone 模型密度熱力圖 | 一張視覺化圖看完 M13a–M17 的 Opus/Sonnet/Haiku 配比，用於預算審核 |
| §25.9 關鍵路徑優先投資建議 | 預算有限時 Opus 投入的優先順序，以及可降級為 Sonnet 的灰色地帶 |
| §5 / §14 / §15 / §16 / §17 heading 行內模型標註 | 過渡方案、Trivy、通知、Observability、失敗模式章節補齊標註 |

---

## v2.2 變更記錄（v2.1 → v2.2 補強）

v2.1 補齊設計細節後，v2.2 補上「誰（哪個模型）來做」的執行層面指引：

| 新增章節 | 解決的問題 |
|----------|------------|
| §25 Claude Code 模型分工建議 | 每個 Milestone / 子任務對應的最佳模型（Opus / Sonnet / Haiku），避免「什麼都用 Opus 燒錢」或「什麼都用 Haiku 翻車」 |
| M13a/M13b/M14/M15/M16/M17 heading 旁的主力模型標註 | 讓開發者打開章節第一眼就看到該 Milestone 的模型配比 |
...
2. [整體架構流程](#2-整體架構流程)
3. [與現有系統的關係](#3-與現有系統的關係)（v2.4: +Argo Rollouts）
4. [現況差距分析](#4-現況差距分析)（v2.4: +灰度發布/Gateway API/Cilium）
5. [近期過渡方案（不需等 M13）](#5-近期過渡方案不需等-m13)
6. [Pipeline 引擎核心設計總覽](#6-pipeline-引擎核心設計總覽)
7. [Pipeline 執行模型（M13 核心）](#7-pipeline-執行模型m13-核心)（v2.4: +§7.4.1 PVC 拓撲約束、§7.7.1 Istio Ambient、§7.7.2 CiliumNetworkPolicy）
8. [M13a — 核心執行引擎（4 週）](#8-m13a--核心執行引擎4-週)
9. [M13b — 進階 Steps 與使用者體驗（4 週）](#9-m13b--進階-steps-與使用者體驗4-週)（v2.4: +§9.4 M13c Argo Rollouts 整合）
10. [M14 — Git 整合與 Webhook 觸發](#10-m14--git-整合與-webhook-觸發)
11. [M15 — 映像 Registry 整合](#11-m15--映像-registry-整合)
12. [M16 — 原生輕量 GitOps（CD）](#12-m16--原生輕量-gitopscd)（v2.4: §12.1 三方邊界重寫）
...
17. [失敗模式與故障恢復](#17-失敗模式與故障恢復)（v2.4: +Rollout 失敗模式）
18. [資料模型總覽](#18-資料模型總覽)
19. [技術選型](#19-技術選型)（v2.4: +§19.1 版本兼容矩陣）
20. [實作路線圖](#20-實作路線圖)（v2.4: +M13c Rollouts 整合 2W）
21. [ADR（架構決策紀錄）](#21-adr架構決策紀錄)（v2.4: +ADR-010 灰度發布策略）
22. [效能 SLA 與容量規劃](#22-效能-sla-與容量規劃)
23. [安全威脅建模（STRIDE-lite）](#23-安全威脅建模stride-lite)
24. [資料遷移腳本示意](#24-資料遷移腳本示意)
25. [Claude Code 模型分工建議](#25-claude-code-模型分工建議)（v2.4: +M13c 模型指派）
26. [附錄 A：Pipeline YAML Schema](#附錄-apipeline-yaml-schema)
27. [附錄 B：與 ARCHITECTURE_REVIEW.md 對應關係](#附錄-b與-architecture_reviewmd-對應關係)
28. [附錄 C：真實 Pipeline 範例集](#附錄-c真實-pipeline-範例集)
29. [附錄 D：Troubleshooting 手冊](#附錄-dtroubleshooting-手冊)
...
> **🤖 主力模型**：Opus 15% / **Sonnet 70%** / Haiku 15%
> PipelineTemplate CRUD 與執行器選擇器用 Sonnet；模板參數驗證（避免注入）用 Opus；模板種子資料與 GitLab CI 範例用 Haiku。詳見 §25.2。
...
#### 7.4.1 vSphere CSI 拓撲約束（v2.4 新增）
...
#### 7.7.1 Istio Ambient 環境注意事項（v2.4 新增）
...
#### 7.7.2 Cilium NetworkPolicy 自動偵測（v2.4 新增）
...
> **🤖 主力模型**：**Opus 40%** / Sonnet 55% / Haiku 5%
> Scheduler loop、JobBuilder SecurityContext、PipelineSecret 加密、版本快照設計皆屬高風險核心元件，需 Opus 主導；CRUD handler/service 實作 Sonnet；種子腳本與基礎設施 yaml Haiku。詳見 §25.2。
...
> **🤖 主力模型**：Opus 20% / **Sonnet 70%** / Haiku 10%
> 進階 Step 類型實作與 UI 頁面主要 Sonnet；Trivy/approval/AI 診斷整合需 Opus 把邊界設計好；i18n 字串與文件補齊 Haiku。詳見 §25.2。
...
### 9.4 M13c — Argo Rollouts 整合（2 週）（v2.4 新增）

> **🤖 主力模型**：**Opus 35%** / Sonnet 60% / Haiku 5%
> Rollout 狀態機整合、Gateway API trafficRouting 對齊需 Opus；Step CRUD 與 UI 面板 Sonnet。
...
> **🤖 主力模型**：**Opus 25%** / Sonnet 70% / Haiku 5%
> Webhook 安全設計（HMAC 驗簽、replay 防護、IP allowlist、rate limit）一律 Opus；GitHub/GitLab/Bitbucket provider 介面與 trigger 規則引擎 Sonnet；路由註冊與 fixture JSON Haiku。詳見 §25.2。
...
> **🤖 主力模型**：Opus 15% / **Sonnet 80%** / Haiku 5%
> Registry CRUD 與 Harbor/ECR/GCR adapter 大多為 API wrapper，Sonnet 為主；credential 加密儲存設計 Opus 把關；DB 表種子與 UI list 頁樣板 Haiku。詳見 §25.2。
...
> **🤖 主力模型**：**Opus 30%** / Sonnet 65% / Haiku 5%
> GitOps diff 引擎、drift detection、reconcile loop、與 ArgoCD 的邊界定義屬核心架構，Opus 主導；Application/Sync/Target 資源 CRUD Sonnet；模板檔與 demo yaml Haiku。詳見 §25.2。

**估計工作量：6 週** | **優先級：🟡 中**

### 12.1 ArgoCD / 原生 GitOps / Argo Rollouts 三方邊界（v2.4 重寫）
...
> **🤖 主力模型**：**Opus 30%** / Sonnet 65% / Haiku 5%
> Promotion 狀態機、環境間 policy 檢查、approval flow 屬關鍵業務邏輯，Opus 主導設計；Environment CRUD 與 promotion history UI Sonnet；demo 範例 Haiku。詳見 §25.2。
...
> **🤖 主力模型**：**Opus 30%** / Sonnet 65% / Haiku 5%
> 雙軌共存期間的資料對齊與 `scan_source` 欄位設計需 Opus；結果查詢層統一與欄位擴充 Sonnet；遷移文件 Haiku。舊 `exec.Command` 廢棄前的 regression 驗證一律 Opus。詳見 §25.2。
...
> **🤖 主力模型**：Opus 20% / **Sonnet 75%** / Haiku 5%
> PipelineRun → NotifyChannel 整合與通知模板用 Sonnet；失敗告警去重演算法（避免通知風暴）用 Opus；文字訊息範本 Haiku。詳見 §25.2。
...
> **🤖 主力模型**：**Opus 30%** / Sonnet 65% / Haiku 5%
> Prometheus metrics label cardinality 控制、histogram bucket 設計屬關鍵決策，Opus 主導；Audit 事件擴充與 K8s events 轉接 Sonnet；PromQL 範例與儀表板 JSON Haiku。詳見 §25.2。
...
> **🤖 主力模型**：**Opus 60%** / Sonnet 35% / Haiku 5%
> 所有 recovery 邏輯（Watcher re-attach、Scheduler 掃描、雙控制器競爭）一律 Opus——失敗處理錯誤 = 資料損毀 / 雙重執行；runbook 文件撰寫 Sonnet；常數表 Haiku。詳見 §25.2。
...
### M13c — Argo Rollouts 整合（2 週）（v2.4 新增）
...
### ADR-010：灰度發布採 Argo Rollouts 代理模式，不自行實作灰度邏輯（v2.4 新增）
...
## 25. Claude Code 模型分工建議

> 本章節是給「使用 Claude Code 實際執行 M13–M17 的工程師」看的操作手冊。
> 目的：讓 CI/CD 引擎這類**高安全、高複雜度、跨系統整合**的專案，不會在「模型挑錯」這件事上額外失血。
> 規則參考 [ARCHITECTURE_REVIEW.md §十五](./ARCHITECTURE_REVIEW.md#十五claude-code-模型分工建議)；本章為 CI/CD 專屬補充。

### 25.1 CI/CD 專屬硬規則

相較於一般後端任務，CI/CD 引擎多了幾項「即使看起來小也必須用 Opus」的檔案類型：

| 檔案類別 | 原因 | 必須用 Opus |
|---------|------|------------|
| `pkg/pipeline/scheduler/*.go` | 並發控制、工作分配邏輯錯一行就 deadlock | ✅ |
| `pkg/pipeline/jobbuilder/*.go` | SecurityContext、Pod 權限、Resource limits 錯誤 = 容器逃逸 | ✅ |
| `internal/services/pipeline_secret*.go` | AES-256-GCM 加解密，錯誤 = 明文洩漏 | ✅ |
| `internal/router/routes_webhook.go` | 公開端點，rate limit / replay / HMAC 錯誤 = 系統被接管 | ✅ |
| `pkg/pipeline/watcher/*.go` | 跨叢集 Informer 狀態機，誤判會丟 Run | ✅ |
| `internal/services/gitops_*.go` | diff 引擎錯誤 = production drift 或反向 overwrite | ✅ |
| `internal/services/rollout_*.go` | 灰度狀態機錯誤 = 誤 promote 或漏 abort | ✅ |
| `pkg/pipeline/version/*.go` | Pipeline 版本快照，snapshot 錯誤 = 歷史不可重現 | ✅ |

**一般 CRUD handler（Pipeline/Registry/Environment）可用 Sonnet**，只要遵循 CLAUDE.md §13 handler 範本即可。

### 25.2 各 Milestone 逐任務模型指派

#### 近期過渡方案（§5，不需等 M13）

| 任務 | 主力模型 | 原因 |
|------|---------|------|
| PipelineTemplate CRUD + DB 表 | **Sonnet** | 標準 CRUD |
| 執行器選擇器（bash / python / node） | **Sonnet** | 現有 Job 建立樣板 |
| 模板參數驗證 | **Opus** | 避免注入攻擊，需嚴格 schema |
| 模板種子資料 | **Haiku** | 純 JSON/YAML |

#### M13a — 核心執行引擎（4 週）

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| Pipeline / PipelineVersion / PipelineRun / StepRun models | **Opus** | DB schema 定案會影響 M13–M17 全部，錯了回不來 |
| Pipeline CRUD + 版本快照邏輯 | **Opus**（快照）+ **Sonnet**（CRUD） | 快照邏輯（immutable、雜湊、回放）需 Opus |
| **PipelineSecret CRUD + AES-256-GCM 加密** | **Opus** 全程 | 密碼學正確性 + key provider 介面 |
| Step Image 白名單設定 | **Sonnet** | 設定頁 + 驗證 |
| **執行佇列 + Scheduler Loop** | **Opus** 全程 | 並發控制 + 佇列飢餓預防 + 公平調度 |
| **JobBuilder（SecurityContext + ResourceLimits + NetworkPolicy）** | **Opus** 全程 | Pod 安全基線，錯一個欄位 = 逃逸風險 |
| JobWatcher（復用 ClusterInformerManager） | **Opus** | 跨叢集狀態機、reconnect、事件 dedup |
| Log 雙層儲存（SSE + pipeline_logs） | **Sonnet** | 既有 SSE 樣板 |
| GC Worker | **Sonnet** | 依 §17 策略 |
| 單元測試 | **Sonnet** | 依測試金字塔 |
| 種子 YAML / fixture | **Haiku** | 純資料 |

**M13a 模型配比**：**Opus ~40%** / Sonnet ~55% / Haiku ~5%。
**關鍵提醒**：M13a 是整個 CI/CD 的地基，Opus 比重不能省。這 4 週如果用 Sonnet 搶進度，M13b~M17 會用 3 倍時間還技術債。

#### M13b — 進階 Steps 與 UX（4 週）

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| `build-jar`（Maven/Gradle） | **Sonnet** | Step 模板化 |
| `trivy-scan` Step | **Opus** | 與 §14 遷移路徑共存 + result_json 結構 |
| `push-image` Step | **Sonnet** | skopeo/crane 包裝 |
| `deploy-helm` Step | **Sonnet** | helm CLI 包裝 |
| `deploy-argocd-sync` Step | **Sonnet** | 呼叫既有 argocd_service |
| **`approval` Step** | **Opus** | 需與 §15 通知、§23 RBAC 整合，審批狀態機 |
| `notify` Step | **Sonnet** | 復用 NotifyChannel |
| Step 級別重試（retry + backoff） | **Opus** | 避免重試風暴，指數退避正確性 |
| 前端 Pipeline 編輯器 UI | **Sonnet** | 表單 + YAML 編輯器 |
| 前端 Run 詳情頁（含 Log 即時串流） | **Sonnet** | 依現有 SSE 頁面樣板 |
| AI 根因分析整合 | **Opus** | 與 AI Chat 服務的 context 設計 |
| i18n + 文件 | **Haiku** | 翻譯字串 |

**M13b 模型配比**：Opus ~20% / **Sonnet ~70%** / Haiku ~10%。

#### M14 — Git 整合與 Webhook（4 週）

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| **Webhook 路由群組（§10.1）** | **Opus** 全程 | 公開端點 = 攻擊面，HMAC/replay/IP allowlist 全要對 |
| GitHub Provider | **Sonnet** | REST API 包裝 |
| GitLab Provider | **Sonnet** | REST API 包裝 |
| Bitbucket Provider | **Sonnet** | REST API 包裝 |
| **Webhook Replay 攻擊防護** | **Opus** | `X-Synapse-Delivery-ID` + 5 分鐘視窗去重 |
| Trigger 規則引擎（path filter / branch / cron） | **Opus** | path 匹配演算法 + cron 解析邊界 |
| Webhook Rate Limit（依 repo） | **Opus** | Redis-backed token bucket 正確性 |
| 觸發 event → PipelineRun 對應 | **Sonnet** | 標準映射 |
| Webhook 管理 UI | **Sonnet** | 表單 + secret 顯示/隱藏 |
| Fixture 測試資料 | **Haiku** | JSON 貼上 |

**M14 模型配比**：**Opus ~25%** / Sonnet ~70% / Haiku ~5%。
**關鍵提醒**：Webhook 是唯一「未授權就能打到內部」的 API，所有安全相關檔案無論多小都要 Opus。

#### M15 — 映像 Registry 整合（3 週）

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| Registry CRUD | **Sonnet** | 標準 |
| **Credential 加密儲存** | **Opus** | 復用 PipelineSecret 加密，確保 key rotation 一致 |
| Harbor Adapter | **Sonnet** | API wrapper |
| ECR Adapter | **Sonnet** | IAM role/token + API wrapper |
| GCR Adapter | **Sonnet** | Workload Identity + API wrapper |
| Registry 列表 UI | **Sonnet** | 表單 + 測試連線 |
| 影像列表 UI | **Sonnet** | 分頁 + 搜尋 |
| Registry 種子資料 | **Haiku** | DB seed |

**M15 模型配比**：Opus ~15% / **Sonnet ~80%** / Haiku ~5%。

#### M16 — 原生輕量 GitOps（6 週）

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| **GitOps Application / Source / Target 資料模型** | **Opus** | 跨 6 週的地基 |
| **Diff 引擎（Git repo YAML vs Live K8s）** | **Opus** 全程 | structural diff + 忽略 managedFields + server-side apply |
| **Drift Detection Worker** | **Opus** | 週期性掃描 + notify + auto-sync 策略 |
| Reconcile Loop | **Opus** | 狀態機、失敗回滾、並發保護 |
| **與 ArgoCD 代理邊界（§12.1）** | **Opus** | 誤判 = 雙寫雙讀，生產事故 |
| Git repo clone + auth | **Sonnet** | go-git 包裝 |
| Application 列表 / 詳情 UI | **Sonnet** | 標準頁面 |
| Manual Sync 按鈕 | **Sonnet** | POST → sync trigger |
| History / Rollback UI | **Sonnet** | 依 Run 頁模板 |
| Fixture git repo | **Haiku** | YAML 範例 |

**M16 模型配比**：**Opus ~30%** / Sonnet ~65% / Haiku ~5%。

#### M17 — 環境管理與 Promotion（5 週）

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| Environment / EnvironmentGroup 資料模型 | **Sonnet** | 依 §18 結構 |
| **Promotion 狀態機** | **Opus** 全程 | dev → staging → prod 的不可逆順序、回滾路徑 |
| **Policy 檢查引擎**（scan result / approval / test pass） | **Opus** | 策略 DSL + 組合條件 |
| Approval Flow 整合 | **Opus** | 與 M13b approval step 共用模型 |
| Promotion History | **Sonnet** | 標準列表 |
| Environment 列表 / 詳情 UI | **Sonnet** | 標準頁面 |
| Promotion 觸發 UI | **Sonnet** | 表單 + 確認彈窗 |
| Demo 範例 | **Haiku** | YAML |

**M17 模型配比**：**Opus ~30%** / Sonnet ~65% / Haiku ~5%。

#### M13c — Argo Rollouts 整合（2 週）（v2.4 新增）

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| **Rollout CRD Discovery 偵測** | **Opus** | Observer Pattern 正確性，避免誤判安裝狀態 |
| **deploy-rollout Step 狀態機** | **Opus** | Rollout 狀態轉換（Progressing → Paused → Healthy / Degraded）需精確映射 |
| rollout-promote / rollout-abort Step | **Sonnet** | 簡單 API 呼叫 |
| rollout-status Step（timeout + on_timeout） | **Opus** | timeout 邊界條件 + abort 觸發邏輯 |
| Rollout CRUD API | **Sonnet** | 標準 handler 模板 |
| RolloutDetail 前端面板 | **Sonnet** | 灰度進度條 + 操作按鈕 |
| RolloutStatusWidget（嵌入 RunDetail） | **Sonnet** | 組件複用 |
| ADR-010 撰寫 | **Opus** | 架構決策留痕 |
| Demo YAML | **Haiku** | 範例資料 |

**M13c 模型配比**：**Opus ~35%** / Sonnet ~60% / Haiku ~5%。

#### §14 Trivy 整合與遷移

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| `trivy-scan` Step（新） | **Opus** | 與舊 trivy_service 共存期的資料對齊 |
| 雙軌寫入（`scan_source` 欄位） | **Sonnet** | 依現有 ImageScanResult 擴欄位 |
| 結果查詢層統一 | **Sonnet** | union query |
| 舊 `exec.Command trivy` 廢棄 | **Opus** | 確認無 regression 才能移除 |
| 遷移文件 | **Sonnet** | runbook |
...
| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| PipelineRun → NotifyChannel 整合 | **Sonnet** | 復用現有 NotifyChannel |
| 通知模板設計 | **Sonnet** | 文字 + 變數 |
| 失敗告警去重 | **Opus** | 同一 Pipeline 短時間多次失敗，避免通知風暴 |

#### §16 Observability

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| Pipeline Metrics（Prometheus） | **Opus** | label cardinality 控制、histogram bucket 選擇 |
| Audit 事件擴充 | **Sonnet** | 復用現有 OperationLog |
| K8s Events 擷取 | **Sonnet** | informer 事件轉接 |

#### §17 失敗模式與故障恢復

| 子任務 | 主力模型 | 原因 |
|--------|---------|------|
| **所有 recovery 邏輯** | **Opus** 全程 | 失敗處理錯誤 = 資料損毀 / 雙重執行 |
| runbook 文件撰寫 | **Sonnet** | 依 §17 格式 |

### 25.3 跨 Milestone 常駐任務

| 任務類型 | 建議模型 | 說明 |
|---------|---------|------|
| **ADR 撰寫（§21 ADR-001~010）** | **Opus** | 架構決策留痕 |
| **STRIDE-lite 威脅建模（§23）** | **Opus** | 安全專業，不可降級 |
| **資料遷移 SQL（§24）** | **Opus** | 冪等性、rollback 腳本、測試 |
| **Pipeline 範例撰寫（附錄 C）** | **Sonnet** | 依 YAML schema |
| **Troubleshooting runbook（附錄 D）** | **Sonnet** | 故障排除五段式 |
| **PR Review（M13 階段）** | **Opus** | 地基階段不可放鬆 |
| **PR Review（M15/M17 階段）** | **Sonnet** | 若僅 CRUD 可降級 |
| **commit message** | **Haiku** | conventional commit |
| **單元測試補齊** | **Sonnet** | 依測試金字塔 |
| **E2E 測試（M13 以上）** | **Opus** | 測試案例設計本身就是安全審查 |

### 25.4 模型切換決策流程（CI/CD 專屬 SOP）

```
1. 這個任務會碰到以下任一嗎？
   - Secrets / crypto / HMAC / JWT
   - Webhook 未授權端點
   - Pod SecurityContext / RBAC
   - 跨叢集狀態機 / Informer
   - 並發控制 / 佇列 / Scheduler
   - GitOps diff / reconcile
   - Rollout 狀態機 / promote / abort
   └ 是 → Opus（一律）
2. 這個任務是 M13a/M14 的核心元件嗎？
   └ 是 → Opus（地基不可降級）
3. 這個任務有現成的模板（CLAUDE.md §13、或本文件 §18 資料模型）？
   └ 是 → Sonnet
4. 這個任務是純 YAML/JSON/範例/文件翻譯嗎？
   └ 是 → Haiku
5. 其他一切情況 → Sonnet（預設值）
```
...
| 技巧 | 說明 |
|------|-----|
| **Opus 寫 1 個完整 Step，Sonnet 推廣其他 Steps** | §9 的 9 種 Step 類型，第一個（例如 `build-jar`）用 Opus 定調樣板（含測試、Log、失敗處理），其餘 8 種讓 Sonnet 複製 pattern |
| **Opus 寫 Scheduler，Sonnet 寫 CRUD** | M13a 4 週內，Opus 只碰 Scheduler/JobBuilder/Secret/Watcher，其餘全 Sonnet |
| **Webhook 用 Opus 寫第一個 Provider，其餘複製** | GitHub/GitLab/Bitbucket 三個 Provider，第一個（GitHub）含完整 HMAC、replay、rate limit，後兩個讓 Sonnet 參照 |
| **Registry Adapter 一律 Sonnet，credential 一律 Opus** | Harbor/ECR/GCR adapter 皆 Sonnet；但 credential encryption 整合一律回到 Opus |
| **Pipeline 範例全 Sonnet，YAML Schema 用 Opus** | 附錄 A Schema 一次性 Opus；附錄 C 四個範例全 Sonnet |
| **禁止 Haiku 碰 `pkg/pipeline/**`** | 無論多小的改動，pipeline 核心套件不給 Haiku |

### 25.6 與 §21 ADR 的對應

§21 列出 10 條 ADR（ADR-001~010），全部建議使用 **Opus** 撰寫。
特別是 ADR-003（Pipeline Secret 加密）、ADR-005（Webhook 安全模型）、ADR-007（GitOps 與 ArgoCD 邊界）、ADR-010（灰度發布策略），這四條錯一條都會導致架構返工。

### 25.7 M13 階段的特別警告

CI/CD 引擎的特殊性：**一旦 M13a 的核心元件（Scheduler / JobBuilder / Watcher / Secret）設計錯誤，M14~M17 會全數受影響**。這與一般後端功能不同（一般後端功能錯了只影響自己）。

因此 M13a 的 **Opus 比重不可低於 40%**。若因為預算限制必須降低成本，寧可延後 M14/M15 開工，也不要在 M13a 省 Opus token。

### 25.8 Milestone 模型密度熱力圖

把 §25.2 的逐任務指派壓縮成可視化圖，方便預算審核與排程會議對齊：

```
          Opus         Sonnet           Haiku
過渡方案   ██         ██████████████   ███      15/70/15  §5 PipelineTemplate
M13a      ████████   ███████████      █        40/55/ 5  核心引擎 ⚠️ 地基
M13b      ████       ██████████████   ██       20/70/10  進階 Steps + UX
M13c      ███████    ████████████     █        35/60/ 5  Rollouts 整合 (v2.4)
M14       █████      ██████████████   █        25/70/ 5  Webhook 安全
M15       ███        ████████████████ █        15/80/ 5  Registry 整合
M16       ██████     █████████████    █        30/65/ 5  GitOps
M17       ██████     █████████████    █        30/65/ 5  Promotion
§14 Trivy ██████     █████████████    █        30/65/ 5  遷移雙軌
§15 通知  ████       ███████████████  █        20/75/ 5  NotifyChannel
§16 Obs   ██████     █████████████    █        30/65/ 5  Metrics/Audit
§17 失敗  ████████████ ███████        █        60/35/ 5  恢復邏輯 ⚠️
```

**關鍵觀察**：

- **M13a（40% Opus）** 與 **§17 失敗模式（60% Opus）** 是全 CI/CD 文件 Opus 比重最高的兩塊，**絕對不可降級**。M13a 是地基，§17 是恢復邏輯——錯一個就是資料損毀。
- **M15（80% Sonnet）** 是 Sonnet 比重最高的 Milestone，Registry Adapter 都是 API wrapper，無需 Opus。
- **Haiku 比重整段很低（5–15%）**：CI/CD 引擎少有「純機械」任務，絕大部分要判斷。

### 25.9 關鍵路徑優先投資建議

若預算有限必須取捨，下表是 Opus 投入的**優先順序**（由高到低）：

| 優先序 | 必保 Opus 投入的任務 | 不投資的代價 |
|-------|---------------------|-------------|
| 🔴 P0 | M13a：Scheduler / JobBuilder / PipelineSecret / JobWatcher | 地基錯會擴散到 M14–M17，整套系統返工 |
| 🔴 P0 | M14：Webhook HMAC / replay / rate limit | 公開端點被接管，整個平台淪陷 |
| 🔴 P0 | §17：Watcher re-attach 與 Scheduler 掃描恢復 | Synapse 重啟 = Run 資料損毀或雙重執行 |
| 🟠 P1 | M13c：Rollout 狀態機整合 + deploy-rollout Step | 灰度操作錯誤 = 生產事故 |
| 🟠 P1 | M16：Diff 引擎 + Drift Detection + ArgoCD 邊界 | Production drift 或反向 overwrite |
| 🟠 P1 | M17：Promotion 狀態機 + Policy 引擎 | 可能跳關、可能反向 promote |
| 🟠 P1 | §21 所有 ADR（10 條）| 6 個月後「為什麼這樣設計」無人能答 |
| 🟡 P2 | §14：Trivy 雙軌遷移 | 可延後，保持現有 `exec.Command` 版本 |
| 🟡 P2 | §16：Metrics 設計（label cardinality） | 可先用寬鬆 label，後期再緊縮 |

**可降級為 Sonnet 的場景**（當 Opus 預算吃緊時）：

- M13b 除 `approval` 與 Step 級別重試外的其他 Step 實作
- M15 的 Harbor/ECR/GCR adapter（第一個用 Opus 出範本後，其餘 Sonnet）
- M14 的 GitLab/Bitbucket Provider（GitHub 用 Opus 出範本後）
- §15 通知整合（除去重演算法）
- 所有 CRUD handler 與 UI 頁面

**絕對不可降級為 Sonnet 的紅線**（即使預算再緊）：

- 任何 `pkg/pipeline/**` 下的檔案
- 任何 `internal/router/routes_webhook.go` 的修改
- 任何 `internal/services/pipeline_secret*.go` / `pipeline_version*.go`
- 任何 `internal/services/gitops_*.go`
- 任何 `internal/services/rollout_*.go`（v2.4 新增）
- 任何 DB migration SQL
- §21 的 10 條 ADR 與 §23 的 STRIDE 威脅建模

### 25.10 檢查清單：M13–M17 模型選擇自檢

```
□ 1. M13a 的 Scheduler/JobBuilder/Secret/Watcher 是不是用 Opus 寫的？
□ 2. Webhook 的 HMAC/replay 防護是不是用 Opus 寫的？
□ 3. GitOps 的 diff 引擎與 drift detection 是不是用 Opus 寫的？
□ 4. Pipeline 範例（附錄 C）是不是用 Sonnet 寫的（Haiku 應該失敗）？
□ 5. i18n 字串與 demo YAML 是不是用 Haiku 寫的（不該用 Opus）？
□ 6. 所有 PR 的 Code Review 是不是至少 Sonnet 等級？
□ 7. `pkg/pipeline/**` 目錄有沒有 Haiku 的 commit？（應該為 0）
□ 8. §21 的 10 條 ADR 是不是全 Opus 撰寫？
□ 9. §17 失敗恢復邏輯（Watcher re-attach、Scheduler 掃描）是不是用 Opus？
□ 10. 我當前 Milestone 的 Opus 比重（見 §25.8 熱力圖）有沒有嚴重偏離？
```
...
**文件結束。** 本版本為 v2.3 設計稿，涵蓋重構（v2.0）+ 實作前補強（v2.1）+ 模型分工（v2.2）+ 跨 Milestone 整合視圖（v2.3）。實作團隊應將本文件與 [ARCHITECTURE_REVIEW.md](./ARCHITECTURE_REVIEW.md) 共同作為 M13–M17 的總綱領，ADR 更動、SLA 調整、威脅模型演進需以 PR 方式回寫本文件。
