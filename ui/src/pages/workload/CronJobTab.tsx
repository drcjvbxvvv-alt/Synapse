...
// 資料狀態
  ...
  const handleMonitor = (record: WorkloadInfo) => {
    navigate(`/clusters/${clusterId}/workloads/${record.namespace}/${record.name}?type=CronJob&tab=monitoring`);
  };

  const handleEdit = (record: WorkloadInfo) => {
    navigate(`/clusters/${clusterId}/workloads/create?type=CronJob&namespace=${record.namespace}&name=${record.name}`);
  };

  const handleRestart = async (record: WorkloadInfo) => {
    try {
      await WorkloadService.restartWorkload(clusterId, record.namespace, record.name, 'CronJob');
