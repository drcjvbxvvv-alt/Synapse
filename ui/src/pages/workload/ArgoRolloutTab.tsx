...
// 資料狀態
  ...
  const loadWorkloads = useCallback(async () => {
    ...
    try {
      ...
      console.error('載入Rollout列表失敗:', error);
  ...
  const handleMonitor = (record: WorkloadInfo) => {
    navigate(`/clusters/${clusterId}/workloads/rollout/${record.namespace}/${record.name}?tab=monitoring`);
  };

  const handleEdit = (record: WorkloadInfo) => {
    navigate(`/clusters/${clusterId}/workloads/create?type=Rollout&namespace=${record.namespace}&name=${record.name}`);
  };

  const handleRestart = async (record: WorkloadInfo) => {
    try {
      await WorkloadService.restartWorkload(clusterId, record.namespace, record.name, 'Rollout');
