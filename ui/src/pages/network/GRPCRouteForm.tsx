...
interface GRPCRouteFormProps {
  open: boolean;
  clusterId: string;
  editing?: GRPCRouteItem | null;
  onClose: () => void;
  onSuccess: () => void;
}

const formToYaml = (values: GRPCRouteFormValues): string => {
  const obj = {
    ...
    spec: {
      ...
      rules: (values.rules || []).map((rule) => ({
        matches: rule.service
          ? [{ method: { service: rule.service, ...(rule.method ? { method: rule.method } : {}) } }]
          : [],
...
const yamlToForm = (yamlStr: string): Partial<GRPCRouteFormValues> => {
  ...
  const rules = ((spec?.rules as Record<string, unknown>[]) ?? []).map((r) => {
    const firstMatch = ((r.matches as Record<string, unknown>[]) ?? [])[0];
    const methodObj = firstMatch?.method as Record<string, string> | undefined;
    return {
      service: methodObj?.service ?? '',
...
const GRPCRouteForm: React.FC<GRPCRouteFormProps> = ({
  ...
  const [form] = Form.useForm<GRPCRouteFormValues>();
  ...
  useEffect(() => {
    ...
    if (editing) {
      gatewayService.getGRPCRouteYAML(clusterId, editing.namespace, editing.name).then((r) => {
  ...
  const handleDryRun = async () => {
    ...
    try {
      ...
      if (isEdit && editing) {
        await gatewayService.updateGRPCRoute(clusterId, editing.namespace, editing.name, yaml, true);
      } else {
        await gatewayService.createGRPCRoute(clusterId, ns, yaml, true);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit && editing) {
        await gatewayService.updateGRPCRoute(clusterId, editing.namespace, editing.name, yaml);
        message.success(t('gatewayapi.messages.updateGRPCRouteSuccess'));
      } else {
        await gatewayService.createGRPCRoute(clusterId, ns, yaml);
      ...
      message.error(msg || t(isEdit ? 'gatewayapi.messages.updateGRPCRouteError' : 'gatewayapi.messages.createGRPCRouteError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const title = isEdit
    ? t('gatewayapi.form.editGRPCRoute', { name: editing!.name })
              ...
              <div style={{ maxHeight: 580, overflowY: 'auto', paddingRight: 4 }}>
                <Form
                  ...
                  initialValues={{
                    ...
                    rules: [{ service: '', method: '', backends: [{ name: '', port: 50051, weight: 1 }] }],
                  }}
                >
                  {/* Basic info */}
                        ...
                        {fields.map((field) => (
                          <Space key={field.key} wrap style={{ display: 'flex', marginBottom: 8 }} align="start">
                            ...
                            <Form.Item
                              ...
                              style={{ marginBottom: 0 }}
                            >
                              <Input style={{ width: 120 }} placeholder="grpc" />
                            </Form.Item>
                      ...
                      <>
                        {ruleFields.map((ruleField, rIdx) => (
                          ...
                          >
                            ...
                            <Space wrap style={{ marginBottom: 12 }} align="baseline">
                              <Form.Item
                                {...ruleField}
                                name={[ruleField.name, 'service']}
                                label={t('gatewayapi.form.grpcService')}
                                rules={[{ required: true, message: t('gatewayapi.form.nameRequired') }]}
                                style={{ marginBottom: 0 }}
                              >
                                <Input
                                  placeholder={t('gatewayapi.form.grpcServicePlaceholder')}
                                  style={{ width: 220 }}
                                />
                              </Form.Item>
                              <Form.Item
                                {...ruleField}
                                name={[ruleField.name, 'method']}
                                label={t('gatewayapi.form.grpcMethod')}
                                style={{ marginBottom: 0 }}
                              >
                                <Input
                                  placeholder={t('gatewayapi.form.grpcMethodPlaceholder')}
                                  style={{ width: 160 }}
                                />
                              </Form.Item>
                            </Space>
                              ...
                              {(backendFields, { add: addBackend, remove: removeBackend }) => (
                                <>
                                  ...
                                  <Button
                                    ...
                                    type="dashed"
                                    icon={<PlusOutlined />}
                                    onClick={() => addBackend({ name: '', port: 50051, weight: 1 })}
                                  >
                        ...
                        <Button
                          ...
                          icon={<PlusOutlined />}
                          onClick={() =>
                            addRule({ service: '', method: '', backends: [{ name: '', port: 50051, weight: 1 }] })
                          }
                        >
