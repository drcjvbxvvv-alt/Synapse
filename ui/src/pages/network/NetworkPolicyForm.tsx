...
const RuleEditor: React.FC<RuleEditorProps> = ({ rules, onChange, direction }) => {
        ...
        >
          ...
          {(rule.type === 'pod' || rule.type === 'namespace') && (
            <div style={{ marginBottom: token.marginXS }}>
                  ...
                  <Col span={10}>
                    <Input
                      ...
                      onChange={e => update(rule.id, {
                        selector: rule.selector.map((p, i) => i === pi ? { ...p, key: e.target.value } : p),
                      })}
                    />
                  </Col>
                  <Col span={10}>
                    <Input
                      ...
                      value={pair.value}
                      onChange={e => update(rule.id, {
                        selector: rule.selector.map((p, i) => i === pi ? { ...p, value: e.target.value } : p),
                      })}
                    />
                  </Col>
                  <Col span={4}>
                    <Button
                      ...
                      size="small"
                      icon={<DeleteOutlined />}
                      onClick={() => update(rule.id, { selector: rule.selector.filter((_, i) => i !== pi) })}
                    />
                  </Col>
                </Row>
              ))}
              <Button
                size="small"
                icon={<PlusOutlined />}
                onClick={() => update(rule.id, { selector: [...rule.selector, { key: '', value: '' }] })}
              >
          ...
          <div>
              ...
              <Row key={pi} gutter={8} style={{ marginBottom: 4 }}>
                ...
                <Col span={4}>
                  <Button
                    ...
                    size="small"
                    icon={<DeleteOutlined />}
                    onClick={() => update(rule.id, { ports: rule.ports.filter((_, i) => i !== pi) })}
                  />
                </Col>
              </Row>
            ))}
            <Button
              size="small"
              icon={<PlusOutlined />}
              onClick={() => update(rule.id, { ports: [...rule.ports, { protocol: 'TCP', port: '' }] })}
            >
